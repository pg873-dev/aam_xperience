...
Welcome again to CyberArk AAM Xperience!   This page contains everything you need for the hands-on session.
...
In this section we integrate Ansible with Conjur and see how we can retrieve credentials from Confur and remove the hardcoded secrets from the Ansible inventory file.
...
We now update the ansible inventory & playbook files that was created earlier and remove the embeded secrets that was hardcoded in the inventory file.
...
It's now time to run the playbook again and observe that the same results can be achieved without hardcoded credentials, and by dynamically retrieving secrets from Conjur. 
